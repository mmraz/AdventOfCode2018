...
data = [map(int, i.split(', ')) for i in open('inputs/in').readlines()]
...
pp.pprint( set(grid[x, max_y-1] for x in range(max_x)))
s = s.union(set(grid[x,       0] for x in range(max_x)))
pp.pprint(set(grid[x,       0] for x in range(max_x))) 
s = s.union(set(grid[max_x-1, y] for y in range(max_y)))
pp.pprint(set(grid[max_x-1, y] for y in range(max_y)))
s = s.union(set(grid[      0, y] for y in range(max_y)))
pp.pprint(set(grid[      0, y] for y in range(max_y)))
